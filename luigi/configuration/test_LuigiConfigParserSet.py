...
class Test_LuigiConfigParserSet:
    ...
    def test_add_section(self):
        ...
        assert config.has_section('NewSection')

    @pytest.mark.regression
    def test_add_option(self):
        config = LuigiConfigParser()
        config.add_section('ExistingSection')
        config.set('ExistingSection', 'new_option', 'value')
        assert config.has_option('ExistingSection', 'new_option')

    @pytest.mark.regression
    def test_update_option(self):
        config = LuigiConfigParser()
        config.add_section('ExistingSection')
        config.set('ExistingSection', 'option', 'old_value')
        config.set('ExistingSection', 'option', 'new_value')
        assert config.get('ExistingSection', 'option') == 'new_value'

    @pytest.mark.regression
    def test_none_value(self):
        config = LuigiConfigParser()
        config.add_section('ExistingSection')
        config.set('ExistingSection', 'option', None)
        assert config.get('ExistingSection', 'option') is None
