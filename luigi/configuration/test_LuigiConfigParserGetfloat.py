...
class Test_LuigiConfigParserGetfloat:
    ...
    def test_getfloat_validfloat(self):
        parser = LuigiConfigParser(BasicInterpolation())
        parser.add_section('test')
        parser.set('test', 'option', '3.14')
        assert parser.getfloat('test', 'option') == 3.14

    @pytest.mark.regression
    def test_getfloat_invalidfloat(self):
        parser = LuigiConfigParser(BasicInterpolation())
        parser.add_section('test')
        parser.set('test', 'option', 'invalid')
        with pytest.raises(ValueError):
            parser.getfloat('test', 'option')

    @pytest.mark.regression
    def test_getfloat_default(self):
        parser = LuigiConfigParser(BasicInterpolation())
        parser.add_section('test')
        assert parser.getfloat('test', 'non_existent_option', default=2.71) == 2.71

    @pytest.mark.regression
    def test_getfloat_nooption_nosection(self):
        parser = LuigiConfigParser(BasicInterpolation())
        with pytest.raises(NoOptionError):
            parser.getfloat('test', 'non_existent_option')

    @pytest.mark.regression
    def test_getfloat_underscore_option(self):
        parser = LuigiConfigParser(BasicInterpolation())
        parser.add_section('test')
        parser.set('test', 'option_with_underscore', '1.618')
        assert parser.getfloat('test', 'option_with_underscore') == 1.618
