...
class Test_LuigiTomlParserGetitem:
    ...
    def test_getitem_existing_key(self):
        ...
        parser.data = data
        assert parser.__getitem__('key1') == 'value1'
    ...
    def test_getitem_return_value_type(self):
        ...
        parser = LuigiTomlParser()
        parser.data = data
        assert isinstance(parser.__getitem__('key1'), int)
        assert isinstance(parser.__getitem__('key2'), int)
        assert isinstance(parser.__getitem__('key3'), int)

    @pytest.mark.regression
    def test_getitem_frozen_data(self):
        data = {'key1': 'value1', 'key2': 'value2'}
        frozen_data = recursively_freeze(data)
        parser = LuigiTomlParser()
        parser.data = frozen_data
        assert parser.__getitem__('key1') == 'value1'
