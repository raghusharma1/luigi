...
class Test_LuigiTomlParserSet:
    ...
    def test_set_with_new_section(self):
        ...
        # Assert
        assert 'new_section' in parser.data
        assert 'option' in parser.data['new_section']
    ...
    def test_set_with_existing_section(self):
        # Arrange
        parser = LuigiTomlParser()
        parser.data = {'existing_section': {}}

        # Act
        parser.set('existing_section', 'new_option', 'new_value')

        # Assert
        assert 'existing_section' in parser.data
        assert 'new_option' in parser.data['existing_section']
        assert parser.data['existing_section']['new_option'] == 'new_value'

    @pytest.mark.positive
    def test_set_with_existing_option(self):
        # Arrange
        parser = LuigiTomlParser()
        parser.data = {'existing_section': {'existing_option': 'old_value'}}

        # Act
        parser.set('existing_section', 'existing_option', 'new_value')

        # Assert
        assert 'existing_section' in parser.data
        assert 'existing_option' in parser.data['existing_section']
        assert parser.data['existing_section']['existing_option'] == 'new_value'
