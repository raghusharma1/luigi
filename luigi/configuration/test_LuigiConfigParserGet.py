...
class Test_LuigiConfigParserGet:
    ...
    def test_get_option_with_underscore(self):
        # Arrange
        config = ConfigParser()
        section, option, value = "section", "option_with_underscore", "value"
        config.add_section(section)
        config.set(section, option, value)

        # Act
        result = LuigiConfigParser.get(config, section, option)
    ...
    def test_get_option_with_dash(self):
        # Arrange
        config = ConfigParser()
        section, option, value = "section", "option-with-dash", "value"
        config.add_section(section)
        config.set(section, option, value)
    ...
    def test_get_no_option(self):
        # Arrange
        config = ConfigParser()
        section, option = "section", "no_option"

        # Act and Assert
        with pytest.raises(NoOptionError):
            LuigiConfigParser.get(config, section, option)

    @pytest.mark.negative
    def test_get_mismatched_default_type(self):
        # Arrange
        config = ConfigParser()
        section, option, default_value = "section", "option", "default_value"
        config.add_section(section)
        config.set(section, option, default_value)

        # Act and Assert
        with pytest.raises(TypeError):
            LuigiConfigParser.get(config, section, option, default=123)
