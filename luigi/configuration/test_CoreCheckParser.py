...
class Test_CoreCheckParser:
    ...
    def test_parser_disabled(self):
        ...
        with pytest.raises(ImportError):
            _check_parser(mock_parser_class, 'any_parser')
    ...
    def test_error_message(self):
        ...
        with pytest.raises(ImportError) as excinfo:
            _check_parser(mock_parser_class, 'any_parser')
        assert 'Parser not installed yet.' in str(excinfo.value)

    @pytest.mark.positive
    def test_different_parser_types(self):
        # Arrange
        mock_parser_classes = [Mock(), Mock()]
        mock_parser_classes[0].enabled = True
        mock_parser_classes[1].enabled = True

        # Act and Assert
        for mock_parser_class in mock_parser_classes:
            try:
                _check_parser(mock_parser_class, 'any_parser')
            except Exception as e:
                pytest.fail(f"Test failed due to unexpected error: {e}")
