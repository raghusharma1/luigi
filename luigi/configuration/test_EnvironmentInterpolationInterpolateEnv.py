...
from exceptions import InterpolationMissingEnvvarError
...
class Test_EnvironmentInterpolationInterpolateEnv:
    ...
    def test_interpolation_existing_envvar(self):
        ...
        config = ConfigParser({"key": "${TEST_ENV_VAR}"})
        environmentInterpolation = EnvironmentInterpolation(config._interpolations)

        # Act
        result = environmentInterpolation._interpolate_env("key", "DEFAULT", "${TEST_ENV_VAR}")
    ...
    def test_interpolation_non_existing_envvar(self):
        # Arrange
        config = ConfigParser({"key": "${NON_EXISTING_ENV_VAR}"})
        environmentInterpolation = EnvironmentInterpolation(config._interpolations)

        # Act and Assert
        with pytest.raises(InterpolationMissingEnvvarError):
            environmentInterpolation._interpolate_env("key", "DEFAULT", "${NON_EXISTING_ENV_VAR}")

    def test_interpolation_multiple_envvar(self):
        # Arrange
        os.environ["TEST_ENV_VAR1"] = "Test Value 1"
        os.environ["TEST_ENV_VAR2"] = "Test Value 2"
        config = ConfigParser({"key": "${TEST_ENV_VAR1} and ${TEST_ENV_VAR2}"})
        environmentInterpolation = EnvironmentInterpolation(config._interpolations)

        # Act
        result = environmentInterpolation._interpolate_env("key", "DEFAULT", "${TEST_ENV_VAR1} and ${TEST_ENV_VAR2}")

        # Assert
        assert result == "Test Value 1 and Test Value 2"

    def test_interpolation_no_envvar(self):
        # Arrange
        config = ConfigParser({"key": "No env var here"})
        environmentInterpolation = EnvironmentInterpolation(config._interpolations)

        # Act
        result = environmentInterpolation._interpolate_env("key", "DEFAULT", "No env var here")
