...
class Test_EnvironmentInterpolationInterpolateEnv:
    ...
    def test_interpolation_existing_envvar(self):
        ...
        config.read_dict({'DEFAULT': {'key': '${EXISTING_VAR}'}})
        assert config.get('DEFAULT', 'key') == 'test_value'
    ...
    def test_interpolation_multiple_envvar(self):
        ...
        config = ConfigParser(interpolation=EnvironmentInterpolation())
        config.read_dict({'DEFAULT': {'key': '${EXISTING_VAR1}${EXISTING_VAR2}'}})
        assert config.get('DEFAULT', 'key') == 'test_value1test_value2'

    @pytest.mark.positive
    def test_interpolation_no_envvar(self):
        config = ConfigParser(interpolation=EnvironmentInterpolation())
        config.read_dict({'DEFAULT': {'key': 'test_value'}})
        assert config.get('DEFAULT', 'key') == 'test_value'
