...
class Test_CoreGetDefaultParser:
    ...
    def test_default_parser_with_env_var(self, monkeypatch):
        monkeypatch.setenv('LUIGI_CONFIG_PARSER', 'cfg')
        assert _get_default_parser() == 'cfg'

    @pytest.mark.negative
    def test_default_parser_with_invalid_env_var(self, monkeypatch, caplog):
        monkeypatch.setenv('LUIGI_CONFIG_PARSER', 'invalid')
        with caplog.at_level(logging.WARNING):
            assert _get_default_parser() == 'cfg'
            assert 'Invalid parser: cfg' in caplog.text

    @pytest.mark.functional
    def test_default_parser_without_env_var(self, monkeypatch):
        monkeypatch.delenv('LUIGI_CONFIG_PARSER', raising=False)
        assert _get_default_parser() == 'cfg'

    @pytest.mark.negative
    def test_default_parser_with_both_invalid(self, monkeypatch, caplog):
        monkeypatch.setenv('LUIGI_CONFIG_PARSER', 'invalid')
        monkeypatch.setitem(os.environ, 'DEFAULT_PARSER', 'invalid')
        with pytest.raises(Exception):
            assert _get_default_parser() == 'invalid'
            assert 'Invalid parser: invalid' in caplog.text
