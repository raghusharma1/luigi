...
class Test_CoreAddConfigPath:
    ...
    def test_add_config_path_file_existence(self, mock_isfile):
        mock_isfile.return_value = False
        with pytest.warns(UserWarning):
            assert add_config_path('non_existent_file.cfg') == False

    @patch('os.path.isfile')
    @patch('configuration.core._get_default_parser')
    @patch('configuration.core._check_parser')
    def test_add_config_path_parser_selection(self, mock_check_parser, mock_get_default_parser, mock_isfile):
        mock_isfile.return_value = True
        mock_get_default_parser.return_value = 'cfg'
        with patch.dict('os.environ', {'LUIGI_CONFIG_PARSER': 'toml'}, clear=True):
            with pytest.warns(UserWarning):
                assert add_config_path('config.toml') == True

    @patch('os.path.isfile')
    @patch('configuration.core._get_default_parser')
    @patch('configuration.core._check_parser')
    def test_add_config_path_unsupported_extension(self, mock_check_parser, mock_get_default_parser, mock_isfile):
        mock_isfile.return_value = True
        mock_get_default_parser.return_value = 'cfg'
        with patch.dict('os.environ', {'LUIGI_CONFIG_PARSER': 'toml'}, clear=True):
            with pytest.warns(UserWarning):
                assert add_config_path('config.txt') == True

    @patch('os.path.isfile')
    @patch('configuration.core._get_default_parser')
    @patch('configuration.core._check_parser')
    def test_add_config_path_parser_not_enabled(self, mock_check_parser, mock_get_default_parser, mock_isfile):
        mock_isfile.return_value = True
        mock_get_default_parser.return_value = 'cfg'
        mock_check_parser.side_effect = ImportError('Parser not installed yet.')
        with patch.dict('os.environ', {'LUIGI_CONFIG_PARSER': 'toml'}, clear=True):
